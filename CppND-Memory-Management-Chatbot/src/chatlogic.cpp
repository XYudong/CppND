...
void ChatLogic::AddAllTokensToElement(std::string tokenID, tokenlist &tokens, T &element)
{
    ...
    while (true)
    {
        // in a loop such that it can add many tokens with the same tokenID to the element
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            newNode = _nodes.end() - 1; // get iterator to last element(the node just emplaced)
